...
**O que faz:** Le performance e gerencia campanhas do Meta
**Precisa de conta:** Sim, conta de anuncios da Meta
**Duas rotas — leia o comparativo antes de escolher:**
`.claude/skills/relatorio-ads/references/fontes-de-dados.md`
- **Conector MCP oficial** (`https://mcp.facebook.com/ads`, adicionar nas configuracoes de
  conectores da conta Claude): conversa e **escreve** (cria/pausa campanha, com aprovacao no
  chat). Nao roda sozinho, nao guarda historico.
- **API direta** (`scripts/meta-ads.py`, token em `scripts/.meta.token.txt`): so leitura, mas
  roda agendada e cospe CSV/JSON pra empilhar historico ou jogar num banco.
