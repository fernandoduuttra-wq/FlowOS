...
name: salvar
description: >
  Salva o trabalho do FlowOS no GitHub (commit + push). Na primeira vez configura o repositório
...
2. **Se opção 1:** rodar `git init`, `git add .`, `git commit -m "Setup inicial do FlowOS"`, `git branch -M main`, `git remote add origin <URL>`, `git push -u origin main`.
