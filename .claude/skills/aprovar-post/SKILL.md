...
- `SITE_URL` no `.env` da raiz — ex: `https://exemplo.com.br`
- Site com deploy automático a partir do `main` do GitHub (Netlify, Vercel, etc.)
- Conta do Instagram Profissional (Comercial ou Criador)
- Token da Meta com permissão de publicação em `scripts/.instagram.token.txt`.
  Conferir com `python scripts/postar-instagram.py checar` — ele diz qual conta o token
  enxerga, quais permissões tem e se dá pra publicar. O cabeçalho do script explica como gerar.
...
Ensaio primeiro (não publica — mostra os slides na ordem, a legenda exata e a conta de destino):

```bash
python scripts/postar-instagram.py marketing/conteudo/<slug>-<data>
```

Conferir a legenda e a ordem. Só então publicar:

```bash
python scripts/postar-instagram.py marketing/conteudo/<slug>-<data> --publicar
```

O script hospeda os PNG sozinho (a Graph API só aceita criativo por URL pública) e devolve o
permalink do post. Se falhar, relatar e parar.

### Passo 8 — Facebook

Ainda manual. Não existe script de Facebook nesse sistema — o `postar-instagram.py` publica só no
Instagram. Mostrar a legenda pro usuário colar, como no passo do LinkedIn abaixo.
